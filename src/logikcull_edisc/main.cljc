(ns logikcull_edisc.main
  "Kotodama WASM entrypoint for the Logikcull_edisc clean-room actor (L4) — Clojure port.
  ...
  facts (`logikcull_edisc.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "logikcull_edisc")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"       :plural "matters"       :id-prefix "logikcul_mat"
    :fields [:clientId :name :status :openedAt]           :required [:name :status]
    :coerce {}                            :refs {}}
   {:entity "Document"     :plural "documents"     :id-prefix "logikcul_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"logikcull_edisc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "logikcull_edisc-compat" :tier tier :entities entities} 200])
...
(defn start! [] :logikcull_edisc-compat/ready)
